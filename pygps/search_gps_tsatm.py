...
def cmdLineParse():
    ...
    parser.add_argument('--height_min', dest='height_min', help='minimumn height')
    parser.add_argument('--height_max', dest='height_max', help='maximumn height')   
...
def main(argv):
    ...
    if inps.height_min:
        H_min =float(inps.height_min)
    else:
        H_min = -9999.0
        
    
    if inps.height_max:
        H_max = float(inps.height_max)
    else:
        H_max = 9999.0
    ...
    for i in range(x):
        ...
        if (dt1 < date1 and dt2 > date2) and  (H_min < float(H) < H_max):    
            k_flag = 1
